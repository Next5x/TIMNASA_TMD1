...
var session = conf.session.replace(/TIMNASA-MD;;;=>/e,"");
...
async function authentification() {
    try {
        ...
        else if (fs.existsSync(__dirname + "/auth/creds.json") && session != "zokk") {            await fs.writeFileSync(__dirname + "/auth/creds.json", atob(session), "utf8");
        }
    }
    catch (e) {
...
setTimeout(() => {
    async function main() {
        ...
        const sockOptions = {
            ...
            browser: ['timnasa tmd', "Linux", "1.0.0"],
...
if (conf.AUTOREACT_STATUS=== "yes") {
    zk.ev.on("messages.upsert", async (m) => {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            console.log("TIMNASA_TMD1 is ONLINE");
        ...
        if (texte.includes('https://') && verifGroupe &&  yes  ) {
         ...
         console.log("lien detecté")
            var veriftimothAdmin = verifGroupe ? admins.includes(idBot) : false;
             ...
             if(superUser || verifAdmin || !veriftimothAdmin  ) { console.log('je fais rien'); return};
                         ...
                         if ( warn >= warnlimit) { 
                          var kikmsg = `TIMNASA-MD  link detected , you will be remove because of reaching warn-limit`;
                             ...
                             await zk.sendMessage(origineMessage, { delete: key });
                              ...
                              var  msg = `TIMNASA_TMD1 Link detected , your warn_count was upgrade ;\n rest : ${rest} `;
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
            let msg = `*TIMNASA_TMD1. 𝐖𝐄𝐋𝐂𝐎𝐌𝐄 𝐈𝐍 𝐓𝐇𝐄 𝐆𝐑𝐎𝐔𝐏 𝐌𝐄𝐒𝐒𝐀𝐆𝐄*`;
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ TIMNASA_TMD1 is connecting...");
            }
            else if (connection === 'open') {
                console.log("✅ TIMNASA_TMD1 Connected to WhatsApp! ☺️");
                console.log("--");
                ...
                console.log("Loading TIMNASA_TMD1 fez ...\n");
                ...
                console.log("fez Installation Completed ✅");

                await activateCrons();
                
                if((conf.DP).toLowerCase() === 'yes') {     

                let cmsg =`      TIMNASA_TMD1 
╭─────────────━┈⊷ 
│🌏 TIMNASA_TMD1 CONNECTED
│💫 ᴘʀᴇғɪx: *[ ${prefixe} ]*
│⭕ ᴍᴏᴅᴇ: *${md}*
 |🆔️ cost: *[ ${ uptime } ]* 
╰─────────────━┈⊷⁠⁠⁠⁠`;
